...
@transaction.atomic
def crear_gimnasio(nombre, email, slug=None):
    """Da de alta un gimnasio y la cuenta staff de su dueño.

    El `User` queda con contraseña INUTILIZABLE a propósito: el staff entra
    por Google, no por contraseña. Además, `PasswordResetForm.get_users()`
    filtra por `has_usable_password()`, así que esta cuenta queda
    automáticamente fuera del reset por mail.
